...
# items_set=pickle.load(open('data/item_set','rb'))
# bundle_item_map=pickle.load(open('data/bundle_item_map','rb'))
# user_bundle_map=pickle.load(open('data/user_bundle_map','rb'))
# user_item_map=pickle.load(open('data/user_item_map','rb'))
# bundle_diversity_map=pickle.load(open('data/bundle_diversity_map','rb'))
# item_data=pickle.load(open('data/all_items','rb'))
# item_id_lookup = pickle.load(open('data/item_id_lookup','rb'))
# item_name_map=pickle.load(open('data/item_name_map','rb'))

group_of_users = []

for i in range(3):
	choice = random.randint(min(user_item_map.keys()), max(user_item_map.keys()))
	if choice not in group_of_users:
		group_of_users.append(choice)
print group_of_users

# all_games_liked = []
# all_bundles = []
# for user in group_of_users:
# 	for game in user_item_map[user]:
# 		if game not in all_games_liked:
# 			all_games_liked.append(game)
# 	for bundle in user_bundle_map[user]:
# 		if bundle not in all_bundles:
# 			all_bundles.append(bundle)

# user_item_map[max(user_item_map.keys())+1] = set(all_games_liked)
# user_bundle_map[max(user_bundle_map.keys())+1] = set(all_bundles)

all_data_bundle=[]
for user,bundles in user_bundle_map.items():
    for bundle in bundles:
        all_data_bundle.append((user, bundle))
...
random.shuffle(all_data_bundle)
data_size=len(all_data_bundle)

# Training data for bundle for bpr bundle model
training_data_bundle=all_data_bundle[:int(0.8*data_size)]
test_data_bundle=all_data_bundle[int(0.8*data_size):]

# Training data for items for bpr_item model
training_data_item=all_item_data[:int(0.8*len(all_item_data))]
test_data_item=all_item_data[int(0.8*len(all_item_data)):]

def check_tuple(tuple_1, tuple_2, map_type):
    return tuple_1[1] not in map_type[tuple_2[0]] and tuple_2[1] not in map_type[tuple_1[0]]

def graph_sampling(n_samples, training_data, map_type):
    sgd_users=[]
    ...
    while n_samples>0:
        ...
        iteration=100
        while not check_tuple(tuple_1, tuple_2, map_type):
            tuple_2=training_data[np.random.randint(len(training_data))]
            iteration-=1
            if iteration == 0:
                break
...
sgd_train_users_items, sgd_train_pos_items, sgd_train_neg_items = graph_sampling(len(training_data_item)*30, training_data_item, user_item_map)
...
test_users, test_pos_items, test_neg_items= get_test_data_items(test_data_item, training_data_item)
...
bpr_item = BPR_Item(10, max(user_item_map.keys())+1, max(items_set)+1)
# bpr_item.train(s_users=sgd_train_users_items, s_pos_items=sgd_train_pos_items, s_neg_items=sgd_train_neg_items)
# bpr_item.test_bundle(test_users, test_pos_items, test_neg_items)
###################
###################
###################
sgd_users, sgd_pos_bundles, sgd_neg_bundles = graph_sampling(len(training_data_bundle)*30, training_data_bundle, user_bundle_map)
        ...
        if app_data[i] in item_data_map:
            for j in range(i+1,l):
                if app_data[j] in item_data_map:
                    count+=1
                    similarity+=jaccard_similarity_score(get_feat(item_data_map[app_data[i]]['tags']), get_feat(item_data_map[app_data[j]]['tags']))
    if count>0:
...
def compute_diversity_latent(app_data, H):
    ...
    for i in range(l):
            for j in range(i+1,l):
                    count+=1
                    # print H[j]
                    similarity += 1 - spatial.distance.cosine(H[i], H[j])
                    # print cosine_similarity(H[i], H[j])
                    # similarity+=cosine_similarity(H[i],H[j])[0]
    if count>0:
        return 1-(similarity/count)
    else:
        return 0.0
...
for bundle,items in bundle_item_map.items():
    bundle_diversity_map[bundle]=compute_diversity_latent(list(items), Gamma)
...
test_users_bundle, test_pos_items_bundle, test_neg_items_bundle, test_n1, test_n2, test_pos_diversity, test_neg_diversity= get_test_data_bundles(test_data_bundle, training_data_bundle, len(items_set))
...
bpr_cold = BPR_Cold(10, max_bundle_size, max(user_bundle_map.keys())+1, max(items_set)+1)
# bpr_cold.train(s_users=sgd_users, s_pos_items=sgd_pos_items, s_neg_items=sgd_neg_items, s_pos_len=sgd_pos_len, s_neg_len=sgd_neg_len, s_pos_diversity=sgd_pos_diversity, s_neg_diversity=sgd_neg_diversity)
# bpr_cold.test_bundle(test_users_bundle, test_pos_items_bundle, test_neg_items_bundle, test_n1, test_n2, test_pos_diversity, test_neg_diversity)

###################
###################
###################

def generate_bundle(items_set, user_set_test, initial_size = 3, max_iteration = 1000, sample_size = 10):
	...
	while iteration < max_iteration:
		...
		for user in user_set_test:
			...
			pref_score = bpr_cold.test_model(user_set, pos_item_set, neg_item_set, pos_item_count, neg_item_count, pos_diversity, neg_diversity)
			user_pref_scores.append(pref_score)

		group_bundle_scores = {}
		average_scores = []
		for score_list in user_pref_scores:
			for i in range(len(score_list)):
				if group_bundle_scores[i] not in group_bundle_scores.keys():
					group_bundle_scores[i] = []
				group_bundle_scores[i].append(score_list[i])

			print group_bundle_scores

		for key, value in group_bundle_scores:
			average_scores.append(np.mean(value))
		...
		if(average_scores[index]>0):
			...
			prob = np.exp(average_scores[index]/T)
			if prob < .00001:
				break
...
print generate_bundle(items_set, group_of_users)
